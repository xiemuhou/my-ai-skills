...
2. **强制全量提交**: 自动分析后必须执行 `git add -A`，把所有已跟踪、未跟踪、删除文件纳入提交范围
...
注意：Review 模式只审核拆分、提交信息和推送，不审核或缩小文件范围；所有工作区改动仍必须通过 `git add -A` 纳入提交。
...
所有模式：
- 必须执行 `git add -A` 暂存所有改动（包括已跟踪文件修改、删除文件、未跟踪文件）
- 不提供跳过自动暂存、排除未跟踪文件、手动指定文件范围的交互
- 若用户要求“只提交部分文件”“排除某文件”“不要提交未跟踪文件”，明确说明本 skill 设计为强制全量提交，并继续按 `git add -A` 处理；除非用户取消本次提交任务
...
5. **绝不** 修改 git config
6. **必须** 使用 `git add -A` 强制暂存全部工作区改动；不得因为文件未跟踪、文件类型可疑、路径不在用户描述范围内而自行排除
