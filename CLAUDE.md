...
个人 Claude Code Skills 集合仓库。每个 skill 是仓库根目录下的独立文件夹，包含 `SKILL.md`（AI 执行规范）和可选的 `README.md`、`config.yaml`。

Skills 通过 `install-xmh-skills` 安装到 `~/.codex/skills/` 和 `~/.claude/skills/` 后全局可用。
...
Skills 放在仓库根目录（而非 `.claude/skills/`），由安装脚本复制到系统级目录。
...
脚本自动扫描根目录下含 `SKILL.md` 的子目录（排除 `install-xmh-skills` 自身），通过 MD5 比较增量安装到 `~/.codex/skills/` 和 `~/.claude/skills/`。

支持参数：`--codex`、`--claude`（单平台）、`--force`（强制重装）、`--dry-run`（预览）。
...
定位 `install-xmh-skills/scripts/install.py` 并执行。用户说"强制"/"更新"时传 `--force`，"预览"/"看看"时传 `--dry-run`，"codex"/"claude"时传对应单平台参数。
